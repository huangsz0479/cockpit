...
1. 顶部工具栏：新建连接、新建查询、打开 SQL 文件、直接执行 SQL 文件和打开设置。
...
清空表、删除表、删除数据库等 UI 操作不可撤销，执行前会要求二次确认。
...
- 点击 **执行 SQL** 选择本地 `.sql` 文件，并在当前目标数据库中直接执行；文件内容不会载入编辑器，也不会显示二次确认。
- SQL 文件采用流式读取，右下角会显示已处理大小、百分比和已执行语句数，并支持取消。内存占用主要取决于单条最大 SQL；超大的单条 `INSERT` 仍需在执行时保留该条语句。
...
2. 如需只执行部分内容，先选中目标 SQL；点击 **执行** 或使用快捷键都只会执行选中文本，未选择文本时则执行编辑器中的全部 SQL。
...
| 新建查询 | `Ctrl+N` | `⌘+N` |
| 打开 SQL 文件 | `Ctrl+O` | `⌘+O` |
| 直接执行 SQL 文件 | `Ctrl+Shift+O` | `⌘+Shift+O` |
| 保存当前 SQL | `Ctrl+S` | `⌘+S` |
...
- 直接执行 SQL（包括 `UPDATE`、`DELETE` 和 DDL）、SQL 文件及恢复任务时不会弹出二次确认；执行前务必核对 SQL、当前连接和目标数据库。
- 通过资源管理器、数据表或表设计器发起的删除、清空操作仍会要求二次确认。
- SQL 中无 `WHERE` 的 `UPDATE` / `DELETE` 以及 `DROP` / `TRUNCATE` 会直接执行，建议先在事务或测试环境中验证语句。
...
- 应用从 GitHub Releases 检查正式版本；发现更新时只会提示，下载和安装需手动完成。
