...
<template><div class="dialog-backdrop" @mousedown.self="emit('close')"><section class="dialog column-manager-dialog" role="dialog" aria-modal="true" aria-labelledby="column-manager-title"><header><div><h2 id="column-manager-title">列管理</h2><p>调整顺序、隐藏字段，并冻结左侧列</p></div><button class="icon-button" aria-label="关闭列管理" @click="emit('close')"><X :size="15" /></button></header><label>冻结前 <input v-model.number="frozenCount" type="number" min="0" :max="order.length" /> 列</label><div class="column-manager-list"><div v-for="(name, index) in order" :key="name"><label><input type="checkbox" :checked="!hidden.includes(name)" :aria-label="`显示字段 ${name}`" @change="toggleHidden(name, ($event.currentTarget as HTMLInputElement).checked)" />{{ name }}</label><button type="button" class="ghost compact icon-only" :aria-label="`上移字段 ${name}`" :disabled="index === 0" @click="move(index, -1)"><ArrowUp :size="14" /></button><button type="button" class="ghost compact icon-only" :aria-label="`下移字段 ${name}`" :disabled="index === order.length - 1" @click="move(index, 1)"><ArrowDown :size="14" /></button></div></div><footer><button class="secondary" @click="emit('close')">取消</button><button class="primary" @click="apply">应用</button></footer></section></div></template>
