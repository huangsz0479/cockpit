...
import { Check, Copy, FileText, LoaderCircle, RefreshCw, X } from "lucide-vue-next";
...
<template>
  <div class="dialog-backdrop" @mousedown.self="emit('close')">
    <section class="dialog diagnostics-dialog" role="dialog" aria-modal="true" aria-labelledby="diagnostics-title">
      <header><div><h2 id="diagnostics-title">诊断日志</h2><p>Cockpit {{ info?.version || '—' }} · 日志中的密码和令牌字段会在读取时脱敏</p></div><button class="icon-button" aria-label="关闭诊断日志" @click="emit('close')"><X :size="15" /></button></header>
      ...
      <footer><span :class="{ 'copy-error': copyState === 'error' }">{{ copyState === 'error' ? '复制失败，请检查剪贴板权限' : '最多显示最新 512 KB' }}</span><button class="secondary" @click="emit('close')">关闭</button></footer>
    </section>
  </div>
</template>
