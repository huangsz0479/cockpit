...
describe("App connection actions", () => {
  ...
  it("opens table details in a dedicated workspace tab", async () => {
    const { app, host, store } = mountApp();
    store.tableDetail = {
    ...
    host.querySelector<HTMLButtonElement>(".detail-toggle")!.click();
    await nextTick();

    expect(host.querySelectorAll(".workspace-tab")).toHaveLength(2);
    expect(host.querySelector(".workspace-tab.active")?.textContent).toContain("表结构 · users");
    expect(host.querySelector(".table-structure-view")?.textContent).toContain("users");
    expect(host.querySelector(".table-structure-view")?.textContent).toContain("id");
    ...
    host.querySelectorAll<HTMLButtonElement>(".table-structure-view .detail-tabs button")[3]!.click();
    await nextTick();
    expect(host.querySelector(".table-structure-view .ddl-view")?.textContent).toContain("CREATE TABLE users");
    app.unmount();
  });
});
